...
if __name__ == '__main__':
    ...
    match response:
        case 'Graduation':
            ...
            grad_vecs = vec_manager.get_vectorstores("Graduation")
            # grad_vecs = [[학점], [교양], [전공], [공학]]      (현재: [[학점],[교양]] )

            for idx, vecs in enumerate(grad_vecs, start = 1):
                # 1. 학점     # 준비) 졸업이수요건(p19~29) 학점
                if idx == 1:
                    ...
                    credit_gpt_response = chain.invoke({
                        'major_type': '단일 전공',  # stu_info 에 존재하지 않음 (klas어디에 있는지 모르겠음)
                ...
                # 2. 교양 gpt
                    ...
                    llm_manager.set_llm_type('grad_liberalArts')  # 교양 gpt  - 아직 template 없음
                    chain = llm_manager.get_chain()
